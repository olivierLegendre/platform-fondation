...
def collect(flows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for node in flows:
        ...
        if node_type == "function" and any(h in name for h in DASHBOARD_NAME_HINTS):
            seeds.add(node_id)
            continue

        if node_type == "postgresql" and any(h in name for h in DASHBOARD_NAME_HINTS):
            seeds.add(node_id)
    ...
    return {
        ...
        "seed_name_hints": list(DASHBOARD_NAME_HINTS),
        "seed_types": sorted(DASHBOARD_TYPES),
        "candidates": items,
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Extract Node-RED dashboard cutover candidates from flows.json"
    )
