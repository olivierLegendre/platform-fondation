...
def collect(flows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for node in flows:
        ...
        if node_type == "function" and any(name.startswith(p) for p in REF_PREFIXES):
            seeds.add(node_id)
    ...
    # Also include one reverse hop: nodes that wire into selected nodes.
    for node in flows:
        node_id = str(node.get("id", ""))
        wires = node.get("wires")
        if not isinstance(wires, list):
        ...
        for outputs in wires:
            if not isinstance(outputs, list):
                continue
            if any(str(t) in selected for t in outputs):
    ...
    return {
        ...
        "seed_prefixes": list(REF_PREFIXES),
        "candidates": items,
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Extract Node-RED reference API cutover candidates from flows.json"
    )
