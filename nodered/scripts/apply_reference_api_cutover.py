...
REF_FUNCTION_PREFIXES = (
    "api_",
    "domain.api.",
    "repo.deviceReference",
    "repo.deviceReferenceMapping",
)
REF_HTTP_PATH_PREFIXES = (
    "/api/v1/device-reference",
    "/api/v1/device-references",
)
DISABLE_TYPES = {"http in", "http response", "function", "postgresql", "json"}
...
def should_disable(node: dict[str, Any]) -> bool:
    ...
    if node_type in {"http in", "http response"}:
        return True

    if node_type == "function":
        if any(name.startswith(prefix) for prefix in REF_FUNCTION_PREFIXES):
            return True
        if "reference" in name.lower():
            return True

    if node_type == "postgresql":
        if "reference" in name.lower() or "mapping" in name.lower():
            return True

    if node_type == "json" and any(url.startswith(prefix) for prefix in REF_HTTP_PATH_PREFIXES):
        return True
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Apply W6-03 reference API cutover preview to Node-RED flows"
    )
    ...
    parser.add_argument(
        ...
        required=True,
        type=Path,
        help="Reference API candidate report JSON path",
    )
    parser.add_argument(
        "--out-flows",
        required=True,
    ...
    report = {
        ...
        "rules": {
            ...
            "function_prefixes": list(REF_FUNCTION_PREFIXES),
            "http_path_prefixes": list(REF_HTTP_PATH_PREFIXES),
        },
    }
