...
SCENARIO_NAME_TOKENS = (
    "scenario",
    "route scenario actions",
    "toggle_command",
)
MQTT_COMMAND_TOPIC_TOKENS = (
    "/set",
    "command",
)
DISABLE_TYPES = {"function", "mqtt out", "postgresql", "ui-template", "ui-group"}
...
def should_disable(node: dict[str, Any]) -> bool:
    ...
    if node_type == "mqtt out":
        return any(token in topic for token in MQTT_COMMAND_TOPIC_TOKENS) or "command" in name

    if node_type == "function":
        return any(token in name for token in SCENARIO_NAME_TOKENS)

    if node_type == "postgresql":
        return "scenario" in name

    if node_type in {"ui-template", "ui-group"}:
        return "scenario" in name
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Apply W6-04 scenario/router cutover preview to Node-RED flows"
    )
    ...
    parser.add_argument(
        ...
        required=True,
        type=Path,
        help="Scenario/router candidate report JSON path",
    )
    parser.add_argument(
        "--out-flows",
        required=True,
    ...
    report = {
        ...
        "rules": {
            ...
            "scenario_name_tokens": list(SCENARIO_NAME_TOKENS),
            "mqtt_command_topic_tokens": list(MQTT_COMMAND_TOPIC_TOKENS),
        },
    }
